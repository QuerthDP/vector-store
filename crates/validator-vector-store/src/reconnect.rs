...
async fn restarting_nodes_only_driver(actors: TestActors) {
    info!("started");
    ...
    for db_ip in db_ips {
        ...
        let rows = results
            .rows::<(i32, Vec<f32>)>()
            .expect("failed to get rows");
        info!("rows after restart of {}: {}", db_ip, rows.rows_remaining());

        // sleep(Duration::from_secs(2)).await;     // <- The sleep fixes the test
    }
