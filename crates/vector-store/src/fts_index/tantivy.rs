...
mod tests {
    ...
    #[tokio::test]
    async fn highlight_batched_matches_per_document_analysis() {
        let table = make_table_with_keys();
        let sender = make_sender(table);
        add_doc(&sender, 1, "fox").await;
        add_doc(&sender, 2, "dog").await;

        let documents = ["quick fox jumps", "turtles swim slowly", "lazy dog sleeps"];
        let batched = highlight(&sender, "fox OR dog", &documents).await;

        let mut per_document = Vec::new();
        for doc in documents {
            let mut result = highlight(&sender, "fox OR dog", &[doc]).await;
            per_document.push(result.remove(0));
        }

        assert_eq!(
            batched, per_document,
            "analyzing the query once for a batch must match analyzing it once per document"
        );
    }
